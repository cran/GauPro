    ...
    k = function(x, y=NULL, beta=self$beta, s2=self$s2, params=NULL) {
      ...
      if (is.null(y)) {
        if (is.matrix(x)) {
          ...
          val <- s2 * corr_exponential_matrix_symC(x, theta)
          return(val)
        } else {
          return(s2 * 1)
        }
      ...
      if (is.matrix(x) & is.matrix(y)) {
        ...
        s2 * corr_exponential_matrixC(x, y, theta)
      } else if (is.matrix(x) & !is.matrix(y)) {
        # apply(x, 1, function(xx) {self$kone(xx, y, theta=theta, s2=s2)})
        s2 * corr_exponential_matrixvecC(x, y, theta)
      } else if (is.matrix(y)) {
        # apply(y, 1, function(yy) {self$kone(yy, x, theta=theta, s2=s2)})
        s2 * corr_exponential_matrixvecC(y, x, theta)
      } else {
